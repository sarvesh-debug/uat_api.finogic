...
<div class="mx-auto w-full max-w-[1200px] px-4 text-[13px]">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        ...
        <div>
            <h2 class="text-xl font-semibold text-gray-800">
               AEPS Transaction Management (Admin)
            </h2>
            <p class="text-gray-500 text-sm">
                Monitor and manage all AEPS transactions
            </p>
        </div>

        <div class="flex gap-2">
            <a href="{{ route('admin.aeps.reports.export', request()->query()) }}"
    ...
    <div class="bg-white shadow-sm border rounded-lg p-5 mb-5">
        ...
        <form method="GET" action="{{ route('admin.aeps.reports') }}"
              class="grid grid-cols-1 md:grid-cols-5 gap-4">

            <input type="text" name="search"
                   value="{{ request('search') }}"
                   placeholder="Search Payment ID / Merchant"
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-100 text-gray-600 uppercase text-[11px]">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left whitespace-nowrap">Service</th>
