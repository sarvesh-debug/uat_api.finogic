...
<head>
  ...
  <title>Finogic | Reset Password</title>
...
<body class="min-h-screen bg-gradient-to-r from-indigo-50 via-blue-50 to-cyan-50 flex items-center justify-center relative overflow-hidden">
  ...
  <div class="w-full max-w-md bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl p-8 relative z-10 animate-fadeIn">
    ...
    <div class="text-center mb-8">
      <h2 class="mt-2 text-3xl font-bold text-gray-800">Reset Password</h2>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Reset Password | Finogic</title>
<script src="https://cdn.tailwindcss.com"></script>
...
<div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
    <div class="text-center mb-6">
        ...
        <h2 class="text-2xl font-bold mt-4">Reset Password</h2>
    ...
    <form action="{{ route('user.reset.password') }}" method="POST" class="space-y-4">
        @csrf
        <input type="hidden" name="token" value="{{ $token }}">
        ...
        <input type="password" name="password_confirmation" placeholder="Confirm Password" required
               class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
        <button type="submit" class="w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition">Reset Password</button>
    </form>
