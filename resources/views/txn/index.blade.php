    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm">
                Monitor and manage payout transactions efficiently
            </p>
        </div>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-100 text-gray-600 uppercase text-[11px]">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center whitespace-nowrap">Pipe</th>
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-gray-50 transition">
                    ...
                    @if ($t->pipe=='Aeronpay')
                        ...
                        class="hidden">
                        @csrf
                        <input type="hidden" name="client_referenceId" value="{{ $t->payment_id }}">
                        <input type="hidden" name="date_of_transaction" value="{{ \Carbon\Carbon::parse($t->created_at)->format('d-m-Y') }}">
...
<div id="refundModal" class="fixed inset-0 bg-black bg-opacity-40 hidden flex items-center justify-center z-50">
    ...
    <div class="bg-white w-full max-w-md rounded-xl shadow-lg p-6">
        ...
        <form id="refundForm" method="POST" action="{{ route('refund.process') }}">
            @csrf
...
function openTxnModal(txn) {
    ...
    //document.getElementById('m_message').innerText = txn.responseBody;

    let messageBox = document.getElementById('m_message');

try {
    let res = JSON.parse(txn.responseBody);
    let message = '';
