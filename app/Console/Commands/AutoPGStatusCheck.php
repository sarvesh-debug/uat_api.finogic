...
{
    protected $signature = 'pg:auto-status';
    protected $description = 'Auto check pending PG payout status and send callback';

    public function handle()
    {
        $this->info("Starting Auto PG Status Check...");

        $transactions = DB::table('pgmanage')
    ...
    ->get();
        ...
        foreach ($transactions as $txn) {
            ...
            try {
                ...
                */
                ...
                Log::info("Auto PG Status Response", [
                    'order_id' => $txn->orderId,
                    'response' => $response
                ]);

                if (empty($response['success']) || empty($response['data'])) {
                    Log::warning("Invalid PG API Response", [
                        'order_id' => $txn->orderId
                    ]);
                    continue;
                }
               ...
               if (in_array($finalStatus, ['SUCCESS','FAILED'])){
                ...
                Log::error("Auto PG Status Error", [
                    'order_id' => $txn->orderId,
                    'error' => $e->getMessage()
                ]);
        ...
        $this->info("Auto PG Status Check Completed.");
    }
}