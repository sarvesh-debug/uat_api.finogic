...
class AutoPG1StatusCheck extends Command
{
    protected $signature = 'pg1:auto-status';
    protected $description = 'Auto check pending PG1 payout status and send callback';

    public function handle()
    {
        $this->info("Starting Auto PG_P1 Status Check...");
    ...
    ->get();
        ...
        foreach ($transactions as $txn) {
            ...
            try {
                ...
                */
                ...
                Log::info("Auto PG_P1 Status Response", [
                    'order_id' => $txn->orderId,
                    'response' => $response
                ]);

                if (empty($response['success']) || empty($response['data'])) {
                    Log::warning("Invalid PG_P1 API Response", [
                        'order_id' => $txn->orderId
                    ]);
                    continue;
                }
               ...
               if (in_array($finalStatus, ['SUCCESS','FAILED'])){
                ...
                Log::error("Auto PG_P1 Status Error", [
                    'order_id' => $txn->orderId,
                    'error' => $e->getMessage()
                ]);
        ...
        $this->info("Auto PG_P1 Status Check Completed.");
    }
}