...
use Carbon\Carbon;
class XpresspayoutController extends Controller
{
    ...
    //dd($charges, $tds);die();
        ...
        DB::table('remittances')
            ...
            ->update(['amount' => $closingBal]);
             ...
             $apiPipe=DB::table('api_pipes')->where('service','PAYOUT')->where('status',1)->first();
            // return $apiPipe;
            // die();

            if($apiPipe->pipe == 'Aeronpay')
                {
                    //return "AeronPay Pipe line active";
                     $bankResponse = [];
        try {
...
//return $bankResponse; die();
           $statusCode = $bankResponse['statusCode'] ?? 500;
                ...
                elseif ($statusCode == '201') {
                    ...
                    DB::table('xpresspayout')
                        ...
                        ->update([
                            ...
                            'status' => 'Pending',
                            'responseBody' => json_encode($bankResponse),
                            'orderId' => $bankResponse['data']['transactionId'] ?? null,
                            'updated_at' => now(),
                        ]);
                     // ✅ Deduct only on success
                    DB::table('users')
        ...
        return response()->json([
            ...
            elseif($apiPipe->pipe == 'Chagan')
                {
                    //return "Chagan Pipe line active";
