...
class PaydrionPayinController extends Controller
{
     protected $paydrion;
    ...
    public function localAuth($apiKey, $merchantId, $request)
    {
        ...
        if ($merchant->PAYIN != 1) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'pay in service inactive'
            ], 403);
        }
    ...
    public function payinPayout(Request $request)
    {

        Log::channel('PaydrionPayin')->info("Request Body Of payin Payout of Paydrion", [
            'ip' => $request->ip(),
            'payload' => $request->all()
        ]);
