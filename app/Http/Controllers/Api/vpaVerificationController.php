...
class vpaVerificationController extends Controller
{
    ...
    if ($merchant->isVPA !== 1) {
        return response()->json([
            ...
            'error'   => [
                ...
                'message' => 'Merchant VPA Verification Service is inactive'
            ],
            'meta' => [
                'merchantId' => $merchantId,
                'timestamp'  => now()
    ...
    public function accountVerify(Request $request)
{
    ...
    try {
      ...
      DB::beginTransaction();
        ...
        $chargeBalance     = 0.95;// Hypothetical charge for account verification
         // Fetch local commissions for the remittance package
            $commissions = DB::table('commissions')
                ...
                ->where('service', 'VPA_VERIFICATION')
                ->get() ?? [];
        ...
        if ($item->service === 'VPA_VERIFICATION' && $amount >= $from && $amount <= $to) {
            // Calculate charges
        ...
        $response = InstantPayHelper::VPAVerify($validated);
        ...
        return response()->json([
            'success' => false,
            'statuscode' => $response['statuscode'] ?? null,
            'message' => $response['status'] ?? 'VPA verification failed',
            'meta' => [
                'request_id'  => $requestId,
                'statuscode'  => $response['statuscode'] ?? null,
