...
use App\Services\ChagansPayment1Service;
...
class pg1Controller extends Controller
{
    ...
    public function pay(Request $request)
    {
       //return $request;die();
        ...
        if ($remittance->pgpayout1==0) {
            return response()->json([
                'status'  => false,
                'message' => 'Payment Gateway pipe1 is not active on your account. Please contact Admin.'
            ], 400);
        }
...
// $finalPg = chagans / chagans1 / chagans2
      ...
      if ($commissions->isEmpty()) {
        ...
        // ---------------------------------------------------------
        $existingTxn = DB::table('pgmanage1')
            ->where('remId', $remittance->remId)
        ...
        $response = $pg->createPaymentRequest(
            ...
            webhook:"https://api.credxpay.com/api/dynamic/v1/pg/callback",
        );
...
public function pgReport(Request $request)
{
    ...
    return view('users.txn.pg1report', compact('txn','summary'));
}
...
public function pgReportAdmin(Request $request)
{
    ...
    return view('txn.pg1Txn', compact('txn','summary'));
}
