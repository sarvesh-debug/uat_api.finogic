...
class PaydrionApiController extends Controller
{
     protected $paydrion;
    ...
    public function localAuth($apiKey, $merchantId, $request)
    {
        ...
        if ($merchant->payout1 != 1) {

            return response()->json([
                'success' => false,
                'message' => 'aeps setement p1 service inactive'
            ], 403);
        }
    ...
    public function impsPayout(Request $request)
    {

        Log::channel('PaydrionImps')->info("Request Body Of Imps Payout of Paydrion", [
            'ip' => $request->ip(),
            'payload' => $request->all()
        ]);
    ...
    //dd($charges, $tds);die();
        ...
        $paymentId  = 'IMPS' . strtoupper(Str::random(10));
        ...
        $bankResponse = [];
        try {
            $bankResponse = $this->paydrion->payoutImps($payload)->json();
                           
            Log::channel('PaydrionImps')->info("Received from Bank Of Imps Payout of Paydrion", [
            'ip' => $request->ip(),
            ...
            if (!empty($bankResponse['code']) && $bankResponse['code'] === 'TXN') {
                DB::table('xpresspayout')
                    ...
                    ->update([
                        ...
                        'orderId'        => $bankResponse['data']['VendorID'] ?? null,
                    ]);
                 ...
                 Log::channel('PaydrionImps')->info("Bank API Error: Imps Payout of Paydrion", [
                'merchant_id'=>$auth->$remId,
                ...
                return response()->json([
                    ...
                    'success'  =>'Initiated',
                    "message"  =>  $bankResponse['mess'] ?? "Payout initiated.",
                    'remId'            => $remittance->remId,
                    'email'            => $remittance->email,
                    'payment_id'       => $paymentId,
